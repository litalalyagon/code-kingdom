...
class Exercise3 extends Exercise {
  ...
  getCodeParts() {
      ...
      if (this.level === 'easy') {
        tree_field = this.createFieldDisplayDetails({pretext: `${englishDict.ex3.ladder} = `, new_line: false});
        bird_field = this.createFieldDisplayDetails({pretext: ` + `}); 
        const combined = tree_field.concat(bird_field);
        return combined;
      }
      else {
        full_field = this.createFieldDisplayDetails({pretext: `${englishDict.ex3.ladder} = `});
      }
      return full_field;
  }
  ...
  extractInputs(selects, inputs) {
    ...
    if (full_string.includes(englishDict.ex3.tree_height_variable)) {
      tree_exists = true;
      full_string = full_string.replace(new RegExp(englishDict.ex3.tree_height_variable, 'g'), String(this.TREE_HEIGHT));
    }
  ...
  isCorrect() {
    if (this.calculation_result < this.TREE_HEIGHT + 3) {
       return { valid: false, message: englishDict.ex3.too_short_error };
    }
    if (this.calculation_result > this.TREE_HEIGHT + 3) {
      return { valid: false, message: englishDict.ex3.too_long_error };
    }
    if (this.calculation_result === this.TREE_HEIGHT + 3) {
  ...
  validate({ selects, inputs }) {
    ...
    if (!tree_exists) {
      return { valid: false, message: englishDict.ex3.error_tree_phrase };
    }

    if (value.includes(englishDict.ex3.biraffe)) {
      return { valid: false, message: englishDict.ex3.biraffe_error};
    }
  ...
  getValidMessage() {
    return englishDict.ex3.success;
  }
}
