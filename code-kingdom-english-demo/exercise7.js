...
class Exercise7 extends Exercise {
  ...
  getCodeParts() {
    ...
    if (this.level === 'easy') {
      key_field = this.createFieldDisplayDetails({ pretext: `${englishDict.if} `, new_line: false });
      color_field = this.createFieldDisplayDetails({ pretext: ` == `, posttext: ':' });
      unlock_line = this.createFieldDisplayDetails({ pretext: `${englishDict.ex7.unlock_line}`, indentation: true, new_line: false, field_type: "text" });
  ...
  extractInputs(inputs) {
    ...
    if (this.level === 'easy') {
      ...
      if_word = englishDict.if;
      symbol_string = '==';
    } else {
      [full_string] = Array.from(inputs).map(s => s.value.trim());
      if (full_string.startsWith(englishDict.if)) {
        full_string = full_string.slice(englishDict.if.length).trim();
        if_word = englishDict.if;
      }
    ...
    if (color_string == englishDict.ex7.key) {
        let temp = key_string;
  ...
  isCorrect() {
    if (this.inputKey === englishDict.ex7.key && this.inputColor === englishDict.colors.blue) {
      return { valid: true, message: englishDict.ex7.success };
    }
    return { valid: false, message: englishDict.ex7.error_but_valid };
  }
  ...
  validate({ inputs }) {
    ...
    if (this.inputIfWord !== englishDict.if) {
      return { valid: false, message: englishDict.ex7.if_error };
    }

    if (!this.inputKey && !this.inputSymbol && !this.inputColor) {
      return { valid: false, message: englishDict.ex7.error_message };
    }

    if (this.inputKey !== englishDict.ex7.key) {
      return { valid: false, message: englishDict.ex7.key_error };
    }

    if (this.inputSymbol !== '==') {
      return { valid: false, message: englishDict.ex7.error_wrong_symbol };
    }

    if (!this.inputColor) {
      return { valid: false, message: englishDict.ex7.color_empty_error };
    }
