...
class Exercise1 extends Exercise {
  ...
  validColors = [
    ...
    englishDict.colors.orange,
    englishDict.colors.black,
  ...
  getErrorMessage(color, clouds, rainbow) {
    ...
    if (!this.validColors.includes(color)) {
      message.push(englishDict.ex1.color_error);
    }
    if (!this.isIntString(clouds) || parseInt(clouds) < 0) {
      message.push(englishDict.ex1.clouds_not_number);
    } else if (!this.validClouds.includes(clouds)) {
      message.push(englishDict.ex1.clouds_error);
    }
    if (!this.validRainbow.includes(rainbow)) {
      message.push(englishDict.ex1.rainbow_error);
    }
    return message.join('<br>');
  }

  isCorrect() {
    return { valid: true, message: englishDict.ex1.success };
  }
}
