...
class Exercise8 extends Exercise {
  ...
  getCodeParts() {
    ...
    if (this.level === 'easy') {
      crown_field = this.createFieldDisplayDetails({pretext: `${englishDict.ex8.crowns} = `});
      flower_field = this.createFieldDisplayDetails({pretext: `${englishDict.ex8.flowers} = `, new_line: false}); 
    }
    else {
  ...
  extractInputs(inputs) {
    ...
    if (this.level === 'easy') {
      ...
      varName1 = englishDict.ex8.crowns;
      varName2 = englishDict.ex8.flowers;      
    }
    else {
  ...
  isCorrect() {
    if (parseInt(this.inputCrowns, 10) > 4 && parseInt(this.inputFlowers, 10) < 2){
      return { valid: true, message: englishDict.ex8.success };
    } else {
      return { valid: false, message: englishDict.ex8.wrong_answer };
    }
  }
  ...
  validate({ inputs }) {
    ...
    const {varName1, val1, varName2, val2} = this.extractInputs(inputs);
     if (!varName1 || !val1 || !varName2 || !val2) {
      ...
      if (varName1 === englishDict.ex8.crowns) {
        crowns = val1;
      } else if (varName1 === englishDict.ex8.flowers) {
        flowers = val1;
      } else {
        return { valid: false, message: englishDict.ex8.failure_var_doesnt_exist };
      }
      // analyze the second input
      if (varName2 === englishDict.ex8.crowns) {
        crowns = val2;
      } else if (varName2 === englishDict.ex8.flowers) {
        flowers = val2;
      } else {
        return { valid: false, message: englishDict.ex8.failure_var_doesnt_exist };
      }
      ...
      if (varName1 === varName2) {
        return { valid: false, message: englishDict.ex8.failure_var_is_missing };
      }
  ...
  getErrorMessage(crowns, flowers) {
    ...
    if (!this.validCrowns.includes(crowns)) {
      message.push(englishDict.ex8.crowns_error);
    }
    if (!this.validFlowers.includes(flowers)) {
      message.push(englishDict.ex8.flowers_error);
    }
