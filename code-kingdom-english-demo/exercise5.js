...
class Exercise5 extends Exercise {
  ...
  getCodeParts() {
    if (this.level === 'easy') {
      return this.createFieldDisplayDetails({ pretext: `${englishDict.ex5.height} = ` });
    } else {
  ...
  extractInputs(inputs) {
    ...
    if (this.level === 'easy') {
      ...
      varName = englishDict.ex5.height;
    }
    else {
  ...
  isCorrect() {
    if (this.inputHeight == 0) {
      return {valid: true, message: englishDict.ex5.success_zero_height};
    }
    return {
      valid: true,
      message: this.isGateOpen(this.inputHeight) ? englishDict.ex5.success_open : englishDict.ex5.success_closed
    };
  }
  ...
  validate({ inputs }) {
    ...
    if (varName !== englishDict.ex5.height) {
      return { valid: false, message: englishDict.ex5.failure_var_doesnt_exist };
    }

    // we should check the value it's all numbers and signs, not letters
    if (!/^[\d.\s\+\-]+$/.test(val)) {
      return { valid: false, message: englishDict.ex5.height_error  };
    }
